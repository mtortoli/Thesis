...
\begin{itemize}
	...
	\item Vengono iterate una serie di operazioni sull'insieme $Clusters$. Poiché è composto da $n$ cluster, si ha che $ \rightarrow O(n)$. L'insieme di queste azioni, a loro volta, hanno una complessità di $O(n)$, in quanto ad ogni iterazione è necessario aggiornare la matrice $D$ ricalcolando la distanza tra i suoi elementi, che sono $n$. In altre parole, vengono eseguite $n$ operazioni $n$ volte. Questo significa che $\rightarrow T(step2)=O(n) \times O(n) = O(n^2)$.
...
\newline
\newline
Grazie a \textbf{Lucia, Anastasia, Serena, Antonio, Diana, Giorgio, Giovanni, Marcella, Marco, Noemi, Renato, Tommaso, Rosanna, Salvatore, Claudia, Emanuele, Francesco e agli altri}. Amici e compagni di "avventure", sono arrivato fin qui grazie anche a voi.
\newline
\newline
