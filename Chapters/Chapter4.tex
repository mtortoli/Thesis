...
\newline
Si prenda in considerazione la seguente matrice \textit{non additiva}:
\[
D = \bordermatrix{\text{specie}&f&b&u&s\cr
                f& 0 & 3 & 4 & 3\cr
                b& 3 & 0 & 4 & 5\cr
...
dove $totalDistance(D_f)=\sum_{k=1}^{n}D(f,k)$ e $totalDistance(D_b)=$\newline$=\sum_{k=1}^{n}D(b,k)$. La sua caratteristica principale è che qualunque sia la matrice delle distanze $D$ in input, l'elemento più piccolo in $D^\star$ corrisponderà \textit{sempre} ad una coppia di foglie vicine nell'albero.